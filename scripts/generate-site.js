...
function rewriteImgSrc(html) {
  // Rewrite relative img src to ../images/<basename>
  return html.replace(/<img\s+[^>]*src=["']([^"']+)["'][^>]*>/gi, (tag, src) => {
    const isExternal = /^(https?:)?\/\//i.test(src);
    const isAbsolute = src.startsWith('/') || src.startsWith('../') || src.startsWith('./');
...
async function build() {
  ...
  for (const img of imageFiles) {
    try { fs.copyFileSync(path.join(rootDir, img), path.join(imgDir, img)); } catch (e) { errors.push(`${img}: ${e.message}`); }
  }
  ...
  // Index
  const indexHtml = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>Student Initiative Group Notes Hub</title><link rel="stylesheet" href="style.css"/></head><body><header><div class="container"><div class="header-content"><h1>📚 Student Initiative Group Notes Hub</h1><p class="subtitle">IGNOU Study Notes & Resources</p></div></div></header><main class="container"><section class="search-section"><input id="search-bar" type="text" placeholder="🔍 Search notes by subject or topic..."/></section><section class="stats"><div class="stat-card"><div class="stat-number">${noteCards.length}</div><div class="stat-label">Total Notes</div></div><div class="stat-card"><div class="stat-number">${new Set(noteCards.map(n => n.subject)).size}</div><div class="stat-label">Subjects</div></div><div class="stat-card"><div class="stat-number">Updated</div><div class="stat-label">${new Date().toLocaleDateString()}</div></div></section><section class="notes-section"><h2>📖 Available Notes</h2><div id="notes-grid" class="notes-grid">${noteCards.map(n=>{const subj=n.subject.replace(/\"/g,'&quot;').toLowerCase();const ttl=n.title.replace(/\"/g,'&quot;').toLowerCase();const link=n.link.replace(/\"/g,'&quot;');return `<div class="note-card" data-subject="${subj}" data-title="${ttl}"><div class="note-header"><span class="subject-badge">${n.subject}</span></div><h3>${n.title}</h3><p class="note-description">${n.description}</p><div class="note-footer"><a class="btn view-btn" href="${link}">View Notes</a></div></div>`;}).join('')}</div></section><section class="about-section"><h2>About This Hub</h2><p>Notes are automatically updated when new content is added to the repository. Share with your friends and study together! 🎓</p></section></main><footer><p>&copy; ${new Date().getFullYear()} Student Initiative Group Notes Hub</p><p>Last updated: ${new Date().toLocaleString()}</p></footer><script>const searchBar=document.getElementById('search-bar');const cards=Array.from(document.querySelectorAll('.note-card'));searchBar.addEventListener('input',e=>{const q=(e.target.value||'').toLowerCase();cards.forEach(c=>{const s=c.dataset.subject||'';const t=c.dataset.title||'';c.style.display=(s.includes(q)||t.includes(q))?'block':'none';});});</script></body></html>`;
