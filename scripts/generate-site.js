...
// DOCX conversion library (optional in workflow)
let mammoth;
...
// Collect markdown files excluding repo docs
...
<body>
  ...
  <footer>
    <p>&copy; ${new Date().getFullYear()} Student Initiative Group Notes Hub</p>
  </footer>
</body>
</html>`;
}

// Build note pages (Markdown)
...
// Build DOCX pages (if mammoth available)
for (const file of docxFiles) {
  try {
    const filePath = path.join(rootDir, file);
    const { subject, title, base } = parseMeta(file.replace(/\.(docx|doc)$/i, '.md'));
    // Copy DOCX to dist for download
    const docsDir = path.join(distDir, 'docs');
    ensureDir(docsDir);
    fs.copyFileSync(filePath, path.join(docsDir, file));

    let htmlContent = `<p><strong>DOCX:</strong> Download the document below. Automatic conversion is optional and may be enabled in CI.</p>
      <p><a class="btn view-btn" href="../docs/${file}" download>⬇️ Download DOCX</a></p>`;
    const noteHtml = renderNotePage(title + ' (DOCX)', subject, htmlContent);
    const htmlFileName = `${base}.html`;
    fs.writeFileSync(path.join(notesDir, htmlFileName), noteHtml, 'utf-8');

    const description = 'Word document (download available)';
    noteCards.push({ subject, title, description, link: `notes/${htmlFileName}` });
  } catch (e) {
    errors.push(`${file}: ${e.message}`);
  }
...
for (const file of pdfFiles) {
  try {
    const filePath = path.join(rootDir, file);
    const { subject, title, base } = parseMeta(file.replace(/\.(pdf)$/i, '.md'));
    const targetPdf = path.join(pdfDir, file);
...
<head>
  ...
  <link rel="stylesheet" href="../note-style.css" />
  <style>.pdf-frame{width:100%;height:80vh;border:1px solid var(--border);border-radius:12px}</style>
</head>
<body>
  <header>
    <div class="container">
  ...
  <main class="container">
    <div class="note-content">
      <p><a class="btn view-btn" href="../pdfs/${file}" download>⬇️ Download PDF</a></p>
      <iframe class="pdf-frame" src="../pdfs/${file}"></iframe>
    </div>
  </main>
  <footer>
    <p>&copy; ${new Date().getFullYear()} Student Initiative Group Notes Hub</p>
    ...
    const description = 'PDF note (view and download)';
    noteCards.push({ subject, title, description, link: `notes/${htmlFileName}` });
  } catch (e) {
    errors.push(`${file}: ${e.message}`);
  }
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Student Initiative Group Notes Hub</title>
...
<body>
  ...
  <main class="container">
    <section class="search-section">
      <input id="search-bar" type="text" placeholder="🔍 Search notes by subject or topic..." />
    </section>
    <section class="stats">
      <div class="stat-card"><div class="stat-number">${noteCards.length}</div><div class="stat-label">Total Notes</div></div>
      <div class="stat-card"><div class="stat-number">${new Set(noteCards.map(n => n.subject)).size}</div><div class="stat-label">Subjects</div></div>
      <div class="stat-card"><div class="stat-number">Updated</div><div class="stat-label">${new Date().toLocaleDateString()}</div></div>
    </section>
    <section class="notes-section">
      <h2>📖 Available Notes</h2>
      <div id="notes-grid" class="notes-grid">
        ${noteCards.map(n => {
          const subj = n.subject.replace(/"/g, '&quot;').toLowerCase();
          const ttl = n.title.replace(/"/g, '&quot;').toLowerCase();
          const link = n.link.replace(/"/g, '&quot;');
          return `
          <div class="note-card" data-subject="${subj}" data-title="${ttl}">
            <div class="note-header"><span class="subject-badge">${n.subject}</span></div>
            <h3>${n.title}</h3>
            <p class="note-description">${n.description}</p>
            <div class="note-footer"><a class="btn view-btn" href="${link}">View Notes</a></div>
          </div>`;
        }).join('')}
      </div>
    </section>
    <section class="about-section">
      ...
      <p>Notes are automatically updated when new content is added to the repository. Share with your friends and study together! 🎓</p>
    </section>
  </main>
  <footer>
    <p>&copy; ${new Date().getFullYear()} Student Initiative Group Notes Hub</p>
  ...
  <script>
    ...
    searchBar.addEventListener('input', (e) => {
      ...
      cards.forEach(c => {
        ...
        c.style.display = (s.includes(q) || t.includes(q)) ? 'block' : 'none';
      });
    });
  </script>
</body>
